...
class Ventilator extends IPSModule {
	...
	private function VerifyDevice() {
		...
		$this->SendDebug(IPS_GetName($this->InstanceID), sprintf(Debug::VERIFYINGIOINSTANCE, $socketId), 0);
		...
		if(json_decode(IPS_GetConfiguration($socketId), true)["Open"]==true) {
			...
			$this->SendDebug(IPS_GetName($this->InstanceID), sprintf(Debug::INSTANCEVERIFIED, $socketId), 0);
			
			return true;
		} else 
			$msg = sprintf(Errors::SOCKETCLOSED, (string) $socketId);	
	...
	private function Send(string $Text, string $ClientIP, int $ClientPort){
		try {
			$this->SendDebug(IPS_GetName($this->InstanceID), sprintf(Debug::SENDINGDATA,  bin2hex($Text)), 0);
			$this->SendDataToParent(json_encode(['DataID' => '{C8792760-65CF-4C53-B5C7-A30FCC84FEFE}', "Type" => 0, "ClientIP" => $ClientIP, "ClientPort" => $ClientPort, "Buffer" => iconv("ISO-8859-1", "UTF-8", $Text)]));
		} catch(Exception $e) {
			$this->LogMessage(sprintf(Errors::UNEXPECTED,  $e->getMessage()), KL_ERROR);
